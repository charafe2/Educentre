  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: spacing.base }} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.subtitle}>Assiduité de {student.firstName}</Text>

        <Card style={{ marginTop: spacing.md }}>
          <View style={styles.summaryRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.bigValue}>{Math.round(ratio * 100)}%</Text>
              <Text style={styles.bigLabel}>Taux de présence</Text>
            </View>
        ...
        {history.length === 0 ? (
          <EmptyState icon="calendar-outline" title="Aucun historique" body="Les présences enregistrées apparaîtront ici." />
        ) : (
